 ...
 *
 * 覆盖的正是 docs/FIX_LOADING_STUCK.md 里那两个 bug：
 *   1. 云函数失败 + 无缓存  → 旧实现 throw（→ 卡加载页）；新实现返回本地会话
 *   2. 已降级的本地会话     → 不再重复发一次必失败的云调用
...
async function main() {
    ...
    // =====================================================================
    {
        ...
        // 模拟冷启动：同一份 storage，新的 service 实例
        const svc2 = new WxAuthService(makeCloud('fail', calls), storage);
        const second = await svc2.login();

        check('第二次 login 未再调用云函数', calls.length === callsAfterFirst, 'calls=' + calls.length);
